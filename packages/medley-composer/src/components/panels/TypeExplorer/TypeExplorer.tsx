...
import { CategorySharp, Folder, FolderOpen, InsertDriveFileOutlined } from "@material-ui/icons";
    ...
    <Fragment>
      ...
      {treeNode.childNodes.length > 0 &&
        treeNode.childNodes.map((tn) => {
          return <TreeItem nodeId={tn.name} key={tn.name} label={tn.name}>
  ...
  return (
    <Observer>
      {() => (
        ...
        >
          {treeItemFromTreeNode(
            buildTreeNodeFromTypes(typeStore.typesActive)
          )}
        </TreeView>
      )}
    </Observer>
