class TypeRepository1 {
    ...
    updateOptions(options) {
        this.onResolvedTypeTreeUpdate = options?.onResolvedTypeTreeUpdate || this.onResolvedTypeTreeUpdate;
...
class CompositionRepository1 {
    ...
    async load(composition, url) {
        if (composition.types.name === undefined) {
            await this.typeRepository.loadFromUrl(new URL(composition.types.toString(), url));
        } else {
    ...
    get composition() {
        ...
        return {
            types: this.typeRepository.typesUrl ? new URL(this.typeRepository.typesUrl.toString()) : this.typeRepository.typeTree || {
            },
            modelsByType: mbt
        };
    }
